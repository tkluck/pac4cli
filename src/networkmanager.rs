...
impl NetworkManager {
    ...
    async fn _get_wpad_info(&self) -> Result<wpad::WPADInfo, dbus::Error> {
        ...
        let active_connections = get_list_of_paths(&self.dbus_conn, "/org/freedesktop/NetworkManager", "org.freedesktop.NetworkManager", "ActiveConnections").await?;
        ...
        for active_connection in active_connections {
            let config_path = get_path(&self.dbus_conn, active_connection.clone(), "org.freedesktop.NetworkManager.Connection.Active", "Dhcp4Config").await?;

            debug!("received config path: {:?}", config_path);
            let options = get_dict(&self.dbus_conn, config_path.clone(), "org.freedesktop.NetworkManager.DHCP4Config", "Options").await?;
            ...
            let config_path = get_path(&self.dbus_conn, active_connection.clone(), "org.freedesktop.NetworkManager.Connection.Active", "Ip4Config").await?;

            debug!("received config path: {:?}", config_path);
            let domains = get_list_of_strings(&self.dbus_conn, config_path.clone(), "org.freedesktop.NetworkManager.IP4Config", "Domains").await?;
